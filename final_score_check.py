...
def run_inference_and_check():
    ...
    try:
        ...
        pattern = r'\[END\] task=(\w+) score=([\d\.]+) steps=(\d+)'
        ...
        all_valid = True
        for task_type, score_str, steps in matches:
            try:
                score = float(score_str)
                if score <= 0.0 or score >= 1.0:
                    print(f"  ❌ {task_type}: score={score} (INVALID - must be strictly between 0 and 1)")
                    all_valid = False
                elif score >= 0.999999:
                    print(f"  ⚠️  {task_type}: score={score} (close to 1, but OK)")
                elif score <= 0.000001:
                    print(f"  ⚠️  {task_type}: score={score} (close to 0, but OK)")
                else:
                    print(f"  ✅ {task_type}: score={score} (valid)")
            except ValueError:
                print(f"  ❌ {task_type}: score={score_str} (not a valid float)")
                all_valid = False
        ...
        problematic_patterns = [
            ...
            r'score=0\.0',
            r'score=0\.000000',
            r'score=1[^\.]',  # score=1 followed by non-dot
            r'score=0[^\.]',  # score=0 followed by non-dot
        ]
...
def main():
    ...
    print("\n" + "=" * 60)
    if success:
        print("✅ All scores are properly clamped! Ready for submission.")
        return 0
    else:
        print("❌ Issues found with scores. Do not submit yet.")
        return 1
